...
(1/2/4 s … ≤ 20 s, up to 4 attempts) on a detected 429, then rethrows so the queue can still retry.
...
The file `src/TeamsNotificationBot/app-requirements.json` declares what this app version needs from infrastructure and the Teams manifest. It is generated by `scripts/generate-requirements.sh` and contains:

- **Runtime version**: .NET version required by the function app
- **Queues**: Storage queue names the app binds to (triggers and writes)
- **Routes**: Well-known HTTP routes consumed by infrastructure (alert webhook endpoint)
- **App settings**: Required function app setting names (flat list)
- **Auth settings**: Platform auth configuration and required app role
- **Bot service**: Bot type and messaging endpoint (used by Terraform to configure Bot Service and derive EasyAuth excluded paths)
- **Teams app configuration**: Manifest-only fields — scopes, notification mode, channel features
- **Teams app command lists**: Bot commands per scope (manifest-only)
- **Infra hash**: Fingerprint of infrastructure-relevant fields for CI change detection

The Terraform module consumes the infrastructure sections (`function_app_runtime_version`, `storage_account_required_queues`, `well_known_routes`, `function_app_required_app_settings`, `bot_auth_settings`, `bot_service`) via its `app_requirements` variable. The manifest script consumes the manifest sections (`notifier_application_version`, `teams_app_configuration`, `teams_app_command_lists`). Extra fields are silently ignored by each consumer.
