...
| Your own curl to `/ingest` gets `403` during testing | The IP filter is enforcing and your IP isn't updown's | Set `UpdownWebhook__IpFilterMode=off` (or `log-only`) for manual verification, then restore `enforce`. See docs/feat-updown-io-webhook/manual-verification.md §8. |
