...
| Symptom | Cause | Fix |
|---------|-------|-----|
| `401 Unauthorized` on `/api/v1/notify/{alias}` | Missing or expired Bearer token | Acquire a fresh token from Entra ID with the correct audience (`api://<api-app-id>`). Tokens expire after 1 hour by default. The 401 comes from EasyAuth: an empty body with `WWW-Authenticate: Bearer realm="…"` and no `X-Correlation-Id`, and nothing about it reaches the app's logs. A 401 *with* `X-Correlation-Id` would come from the app, which means platform authentication is off (e.g. local development). |
...
| Want to confirm what source IPs updown uses | Observing before enforcing | Temporarily set `IpFilterMode=log-only` (the default is `enforce`), send updown's test payload, and read the `SourceIp=` log lines (App Insights `traces`); confirm they appear via `show-ip-allow-list updown`, then remove the override to return to `enforce`. |
| Cards lag behind a burst of updown events | Bot Framework throttles the bot's **outbound** sends (`ReplyToActivity … '(429) TooManyRequests' … Throttled` in `traces`) | Expected + self-healing: the app retries each send with capped exponential backoff, then the queue retries. No action for short bursts. If cards land in the poison queue, the throttle outlasted the retry budget — `queue-retry notifications-poison` once it clears. |
| Webhook token appears in App Insights | The `{token}` is redacted in worker traces, but the **host-generated** `requests` telemetry (`requests.url`) can still contain it — a worker-registered initializer can't reach host telemetry | Restrict **App Insights read access**; rotate any exposed webhook with `rotate-webhook <id>`. (See [authentication.md](authentication.md) §5, "Webhook Token Authentication".) |
