...
//
// Copied from: https://github.com/microsoft/Agents/blob/main/samples/dotnet/quickstart/AspNetExtensions.cs
// Provides AddAgentAspNetAuthentication() for inbound JWT validation of Bot Framework and Entra ID tokens.
    ...
    public static void AddAgentAspNetAuthentication(this IServiceCollection services, TokenValidationOptions validationOptions)
    {
        ...
        .AddJwtBearer(options =>
        {
            ...
            options.Events = new JwtBearerEvents
            {
                ...
                {
                    var logger = context.HttpContext.RequestServices.GetRequiredService<ILoggerFactory>().CreateLogger("AspNetExtensions.JwtBearer");
                    var claimsPrincipal = context.Principal;
                    var issuer = claimsPrincipal?.FindFirst("iss")?.Value ?? "unknown";
                    var appId = claimsPrincipal?.FindFirst("appid")?.Value ?? claimsPrincipal?.FindFirst("azp")?.Value ?? "unknown";
                    logger.LogInformation("JWT validated: issuer={Issuer}, appId={AppId}, audience={Audience}",
                        issuer, appId, context.Options.TokenValidationParameters.ValidAudiences?.FirstOrDefault() ?? "unknown");
                    return Task.CompletedTask;
                },
                OnForbidden = context =>
                {
                    var logger = context.HttpContext.RequestServices.GetRequiredService<ILoggerFactory>().CreateLogger("AspNetExtensions.JwtBearer");
                    logger.LogWarning("JWT forbidden: {Result}", context.Result?.ToString() ?? "no details");
                    return Task.CompletedTask;
                },
                OnAuthenticationFailed = context =>
                {
                    var logger = context.HttpContext.RequestServices.GetRequiredService<ILoggerFactory>().CreateLogger("AspNetExtensions.JwtBearer");
                    logger.LogError(context.Exception, "JWT authentication failed: {Message}", context.Exception?.Message ?? "no details");
                    return Task.CompletedTask;
                }
            };
        });
