...
- **`TeamsBotHandler` is an `AgentApplication` with routes, not an `ActivityHandler`.** Migrated 2026-08-24 (`docs/contributing.md` §9 has the record). Routes are registered in the constructor; `AgentApplicationOptions` comes from DI with SDK-side mention handling and the typing timer explicitly off — mention stripping happens inside the handler. Member added/removed events are deliberately unrouted (the old model needed no-op overrides to suppress per-member HTTP calls; the new one just has no route). Handler tests construct it with `TestAgentOptions.Create()` and every mocked `ITurnContext` must stub `Services` AND `StackState` with real `TurnContextStateCollection`s, or the turn pipeline NREs.
